...
fn resolve_vector_store(
    ...
    match choice.unwrap_or("local") {
        ...
        other => Err(format!("unknown MEMORIA_VECTOR_STORE value {other:?} (expected \"local\", \"sqlite\", \"postgres\", or \"qdrant\")")),
    }
}
...
mod tests {
    ...
    fn resolve_vector_store_defaults_to_local_and_persists_json_snapshot() {
        ...
        let (_, persist_json_snapshot, label) =
            resolve_vector_store(None, &path, &dir.join("unused.db"), NetworkedStoreEnv::default(), NetworkedStoreEnv::default()).expect("expected a store");
    ...
    fn resolve_vector_store_rejects_an_unknown_choice() {
        ...
        assert!(resolve_vector_store(Some("bogus"), &dir.join("a.json"), &dir.join("a.db"), NetworkedStoreEnv::default(), NetworkedStoreEnv::default()).is_err());
    }
    ...
    fn resolve_vector_store_sqlite_choice_fails_clearly_without_the_feature() {
        let dir = std::env::temp_dir();
        assert!(resolve_vector_store(Some("sqlite"), &dir.join("a.json"), &dir.join("a.db"), NetworkedStoreEnv::default(), NetworkedStoreEnv::default()).is_err());
    }
    ...
    fn resolve_vector_store_sqlite_choice_builds_and_does_not_persist_json_snapshot() {
        ...
        let (_, persist_json_snapshot, label) =
            resolve_vector_store(Some("sqlite"), &dir.join("unused.json"), &db_path, NetworkedStoreEnv::default(), NetworkedStoreEnv::default()).expect("expected a store");
    ...
    fn resolve_vector_store_postgres_choice_fails_clearly_without_the_feature() {
        ...
        assert!(resolve_vector_store(Some("postgres"), &dir.join("a.json"), &dir.join("a.db"), postgres, NetworkedStoreEnv::default()).is_err());
    }
    ...
    fn resolve_vector_store_postgres_choice_requires_url() {
        ...
        let err = resolve_vector_store(Some("postgres"), &dir.join("a.json"), &dir.join("a.db"), postgres, NetworkedStoreEnv::default()).err().expect("expected an error");
    ...
    fn resolve_vector_store_qdrant_choice_fails_clearly_without_the_feature() {
        ...
        assert!(resolve_vector_store(Some("qdrant"), &dir.join("a.json"), &dir.join("a.db"), NetworkedStoreEnv::default(), qdrant).is_err());
    }
    ...
    fn resolve_vector_store_qdrant_choice_requires_url() {
        ...
        let err = resolve_vector_store(Some("qdrant"), &dir.join("a.json"), &dir.join("a.db"), NetworkedStoreEnv::default(), qdrant).err().expect("expected an error");
